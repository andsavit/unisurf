...
def calculate_name_score(prof_name, author_name):
    ...
    cognome_score = JaroWinkler.similarity(prof_cognome, author_cognome) * 100
...
def calculate_token_set_score(prof_tokens, author_tokens):
    ...
    Returns:
        float: Score da 0 a 100
    """
    # Normalizza i token per il confronto
    prof_tokens_clean = [token.lower() for token in prof_tokens if token.isalpha()]
    author_tokens_clean = [token.lower() for token in author_tokens if token.isalpha()]
    ...
    initial_bonus = 0
    for prof_token in prof_tokens_clean:
        for author_token in author_tokens_clean:
            if is_initial(author_token):
                initial = extract_initial(author_token).lower()
                if prof_token.startswith(initial):
                    initial_bonus += 10
            elif is_initial(prof_token):
                initial = extract_initial(prof_token).lower()
                if author_token.startswith(initial):
                    initial_bonus += 10
...
def calculate_first_name_score(prof_nome, author_first):
    ...
    jaro_score = JaroWinkler.similarity(prof_nome, author_first) * 100
